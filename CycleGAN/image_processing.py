...
class NucleiDataset(data.Dataset):
    ...
    def __init__(self, image_dir, label_dir, transform=None):
        ...
        self.index_map = dict([(i, os.path.splitext(file)[0])
...
class CezanneDataset(data.Dataset):
    """
    ...
    def __init__(self, im_dir, transform=None):
        ...
        self.transform = transform
        self.index_map = dict([(i, os.path.splitext(file)[0])
                               for i, file in enumerate(os.listdir(im_dir))])

    def __len__(self):
        return len(os.listdir(self.im_dir))

    def __getitem__(self, index):
        image = Image.open(self.im_dir + "/" + self.index_map[index] + ".jpg")
        if self.transform:
